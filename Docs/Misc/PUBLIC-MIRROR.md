...
of endpoints is "source is open for review" â€” that's what makes the offering
acceptable for IT operators.

The private repo carries operational content that must not leak publicly:

- Production server IPs (TOASTWEB1 `<your-web-server-ip>`, TOASTDATA1 `<your-db-private-ip>`).
- SSH key file references (`Docs/Assets/*.pem` â€” already `.gitignore`d as
...
- `src/` â€” all source for `ToastRevival.Api` and `ToastRevival.Dashboard`.
- `tests/` â€” all test projects.
- `installer/` â€” WiX installer source (signing certs are external).
- `scripts/build-*.ps1` â€” build scripts a self-hoster needs to compile and sign
  their own agent (the self-host signing path in `Docs/rollyourown.md`). We do
  not ship a pre-signed agent â€” self-hosters sign with their own certificate.
- `docker-compose.yml` and the two `Dockerfile`s â€” the self-host stack.
- `README-SELF-HOST.md` â€” the operator-facing README.
- `Docs/Misc/rollyourown.md` â€” open-core strategy doc; public-facing already.
- `Docs/Misc/PUBLIC-MIRROR.md` â€” this file. The public mirror documents its own
  origin honestly.
- `CONTRIBUTORS.md`, `LICENSE`, `.gitignore`, `global.json`, `NuGet.config`,
  `ToastRevival.sln`.
- `.github/workflows/agent-build.yml`, `api-tests.yml`, `codeql.yml` â€” CI
...
- `Docs/ToastRevival/DEPLOY.md` (replaced on public side with a sanitized
  `Docs/DEPLOY-SELFHOST.md` if one is needed â€” but the bulk of self-host
...
- `Docs/ToastRevival/CHANGES-*.md` â€” internal change tracking
- `Docs/Assets/*.pem` â€” already `.gitignore`d
- `.docpro/` â€” team memory and session state
- `infrastructure/` â€” production-specific nginx/systemd; replaced with a
  sanitized `infrastructure/self-host/` subset if needed
- `publish/`, `dist/`, `bin/`, `obj/`, `artifacts/` â€” build artifacts
...
git tag -a v0.4.8 -m "0.4.8 â€” Server 2025 toast banner fix"
...
Dry-run runs steps 1â€“5, then prints the public-side `git status` and
...
automatically on the next sync â€” no manifest edit needed.
...
- [ ] No files under `Docs/ToastRevival/` exist in the public worktree
      ...
      `Docs/Misc/PUBLIC-MIRROR.md` ship at `Docs/Misc/`).
...
- **Squash vs. preserve.** Right now every public commit corresponds to a
  ...
  history. That's deliberate â€” private commit history contains AI handoff
