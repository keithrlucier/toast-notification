...
- **D3** [BUILT 2026-05-08, awaiting sign + lab verify]: MSI wrapper installs the agent + registers `\Toast2IT\ToastNotificationAgentLogon` Scheduled Task at install (replaces M0A's Startup-folder shortcut). Task is logon-triggered with BUILTIN\Users group principal (`S-1-5-32-545`) at `LeastPrivilege`; action runs `%ProgramFiles%\Toast Notification\ToastNotification.Agent.exe --template alert --no-wait`. WiX 5 deferred custom actions invoke `[System64Folder]schtasks.exe` with `/Create /XML /F` (after InstallFiles, condition `NOT REMOVE`) and `/Delete /F` (before RemoveFiles, condition `REMOVE="ALL"`, `Return="ignore"` for idempotency). Pre-install verification clean: XML parses (UTF-16 LE BOM matching schtasks /Query /XML export format), MSI Custom Action table correct (Type 3106 install / 3170 uninstall, deferred + non-impersonate), payload byte-identical from repo → cab → admin-install extract. StartupShortcut component dropped from WiX. See `EVIDENCE/2026-05-08-m0-d3-msi-build-with-scheduled-task.md`. Hand-off: Keith signs `ToastNotification.Agent-0.3.0.0.msi`, installs on Win11 lab, runs `Get-ScheduledTask -TaskPath '\Toast2IT\' -TaskName 'ToastNotificationAgentLogon'` to verify, logs out and back in to verify toast fires from the task, captures EVIDENCE entry.
