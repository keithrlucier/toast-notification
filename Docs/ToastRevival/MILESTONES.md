...
- **D7** [x **COMPLETE 2026-05-13**]: Marketing site pricing page rewrite. Diana leads. `Pricing.tsx` fully rewritten — origin story (2020 MSP COVID build, 986k notifications, 17 tenants) leads the page, three equal-weight `m-tier-card` columns (Free Trial / Managed SaaS / Roll Your Own — none "featured" because none of them is the recommended path, they serve different operators), no comparison table, no urgency CTAs, RYO card links to `https://github.com/keithrlucier/toast-notification`. FAQ rewritten to the 3-tier model with honest Path A vs Path B signing guidance, 14-day cliff behavior, and RDS device counting. `Home.tsx` pricing teaser converted to `m-tier-grid` layout; legacy ✓ checkmark Unicode glyphs removed from UI bullets per Diana's standing rule. Aligned crawler-facing surfaces: `Llms.tsx`, `llms.txt`, `scripts/prerender-seo.mjs` (route descriptions + bodies + JSON-LD), `src/lib/seo.ts` (`softwareApplicationLd` Offer renamed Standard→Managed SaaS with `eligibleQuantity` corrected to 0–100; `pricingProductLd` rewritten as `AggregateOffer` with 3 Offers). Build clean (tsc + Vite + prerender, 0 errors, existing chunk warning unchanged). Grep verification: zero remaining "26-100 devices" / "101-200 devices" / "$44/month" / "fleet block" patterns in dist. Code Sweep #41 (Abish) SHIP verdict. Two INFO items filed (INFO-M11-D7-001 Home.tsx terminal-comparison glyphs deferred for Diana; INFO-M11-D7-002 AggregateOffer lowPrice $0 informational).
...
- **INFO-M11-SW-001 TOCTOU on trial registration** [x]: New `LicenseService.TryRegisterDeviceAtomicAsync` wraps the cap check + device INSERT + ConsumedCount bump in a transaction gated by `pg_advisory_xact_lock` keyed on `tenantId.GetHashCode()`. Tenant is reloaded inside the lock so the check sees authoritative state. `DevicesController.Register` calls the new method; Stripe sync stays outside the transaction. Regression test (`TrialDeviceCapConcurrencyTests.Register_ConcurrentTrialBurst_NeverExceedsTrialCap`) spins up a fresh `ApiTestFactory` with `TOAST_REQUIRE_BILLING=true`, fires two concurrent registers at cap-minus-one, asserts exactly one 200 + one 403 + final `ConsumedCount`=2. See `git log --grep "INFO-M11-SW-001"`.
...
- Agent code signing: **RESOLVED (policy revised 2026-05-22, Keith)** — self-hosters MUST acquire their own OV cert (~$300-400/yr, 1-3 day validation) and sign the MSI themselves. We do NOT distribute a pre-signed agent for self-host; our Toast2IT-signed agent is delivered only to paying managed subscribers from their portal and is never to run on a self-hosted instance. The former "Path A — use our pre-signed MSI from GitHub Releases" offering was removed as policy-violating. Signing friction is intentional — it converts evaluators to SaaS. Documented in `README-SELF-HOST.md` and `Docs/rollyourown.md`.
...
- D7 sweep (Abish, 2026-05-13, Sweep #41): Marketing surface rewrite across 6 files (Pricing.tsx, Home.tsx, Llms.tsx, llms.txt, seo.ts, prerender-seo.mjs). Caught crawler-surface drift mid-sweep — prerender bodies and AggregateOffer shapes still on old block pricing — patched before commit. Build + grep verification clean. Two INFO items filed (-001 ✓ glyph deferred, -002 AggregateOffer $0 informational). SHIP.
- D2–D6: sweeps embedded in commits 76ae2ef / a5b9a8a / eb388b0 (no separate sweep entries — fixes were scoped, additive, and verified in-line).
