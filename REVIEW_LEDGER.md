...
**In-code anchors — BLOCKED this session (tool-integrity).** Per the no-defer rule, the two resolved-as-safe agent findings (Agent-L1/L2) and the deferred DASH-L1 each need an in-code anchor comment (finding ID + decision) so the next cold sweep does not re-flag them. Those anchors live in `SelfUpdateService.cs` and `DeviceAppearanceCards.tsx`. The Read tool fabricated/garbled the contents of BOTH files this session (git proves the real files are clean and unchanged: working tree == HEAD, `git show HEAD:` returns valid source; the Read tool returned literal garbage e.g. `mfwccc 4cccchnmvkkfl...` at SelfUpdateService.cs:316). `Edit` requires a trustworthy `old_string`; editing a security-critical file off a fabricating read is forbidden by standing rule. **Anchors are therefore deferred to a fresh session with a trustworthy read** — this is the rare legitimate deferral (a tool-integrity blocker, not our execution). Tracked as ANCHOR-PENDING below.
...
| DASH-L1 | Low | DEFERRED (owner: Keith — DB schema) · ANCHOR-PENDING | `src/ToastRevival.Dashboard/src/components/DeviceAppearanceCards.tsx:299-302` | Lock-screen cache-bust key is `mountTime (Date.now() at mount) + cacheBust counter`, not a server-provided version. The visible bug (stale preview after replace) **is already remediated** for the uploading admin; but the "persist across navigation" commit actually *regenerates* the buster each mount (mild over-fetch, no true persistence), and nothing is keyed to a value that provably changes on replace for other viewers / CDN. | Low real impact: no service worker; no CDN in repo; over-fetch of one small preview image is negligible. The robust fix (add `LockScreenImageUpdatedAt` to `Tenant`, surface in `LockScreenConfigResponse`, use as `?v=`) is a **DB-schema change = Keith's architectural call**. Blocking decision: whether to add the column now or accept current behavior. | High |
| Agent-L1 | Low | REMEDIATED (safe-by-design) · ANCHOR-PENDING | `src/ToastRevival.Agent/SelfUpdateService.cs:310` (`LockDownToSystemAndAdmins`) | Staging-dir ACL is set at runtime in C#, not declared in WiX — contrary to the standing "ACLs belong in WiX" rule. | Safe here: the user writes to `update\`, not `verified\`; the SYSTEM updater re-creates + re-locks + reparse-checks `verified\` every run before use, so a pre-seeded ACL is not load-bearing. Optional future hardening: declare the staging dirs in WiX with `util:PermissionEx`. Resolved no-change; in-code anchor pending (see ANCHOR-PENDING). | High |
| Agent-L2 | Low | REMEDIATED (safe-by-design) · ANCHOR-PENDING | `src/ToastRevival.Agent/SelfUpdateService.cs:170,499` | Trigger file (`pending-action.txt`) lives in user-writable ProgramData root; a local non-admin can write it. | Contained by downstream validation: uninstall arg is GUID-regex-validated; update arg is a path copied into the protected dir and Authenticode-re-verified before any execution — a forged path can at most point at an unsigned MSI, which fails the signature gate. No escalation. Resolved no-change; in-code anchor pending (see ANCHOR-PENDING). | High |

---

## ANCHOR-PENDING (next session, trustworthy read required)

These dispositions are final; only the in-code anchor comment is outstanding. Add each anchor (finding ID + decision) so a future cold sweep does not re-flag the finding, then strike it from this list.

| ID | File | Anchor to add |
|----|------|---------------|
| Agent-L1 | `src/ToastRevival.Agent/SelfUpdateService.cs` @ `LockDownToSystemAndAdmins` | `// REVIEW Agent-L1 (2026-05-31): ACL set in C# (not WiX) is intentional — verified\ is SYSTEM/Admin-owned and re-locked+reparse-checked every run; pre-seeded ACL not load-bearing.` |
| Agent-L2 | `src/ToastRevival.Agent/SelfUpdateService.cs` @ trigger-file read | `// REVIEW Agent-L2 (2026-05-31): trigger file in user-writable ProgramData is safe — uninstall arg is GUID-validated; update path is Authenticode-re-verified on the protected copy before execution.` |
| DASH-L1 | `src/ToastRevival.Dashboard/src/components/DeviceAppearanceCards.tsx` @ cache-bust state | `// REVIEW DASH-L1 (2026-05-31): cache key is mountTime+counter, not server-versioned. Robust fix (LockScreenImageUpdatedAt column) deferred to Keith's DB-schema call; visible symptom already remediated.` |

**Why pending:** the Read tool fabricated/garbled both source files this session (git-verified clean on disk). Anchoring via `Edit` needs a trustworthy `old_string`; deferred to a fresh session rather than risk a corrupt edit to a security-critical file.
