...
.DESCRIPTION
    ...
    to delete the scheduled tasks the new MSI had just created.

    Symptom: ToastNotification.Agent.exe is present in C:\Program Files\Toast Notification
    but the device shows offline in the tenant dashboard because no scheduled task fires
    at user logon.

    This script re-creates both tasks from the XML files already on disk, then fires the
    logon task so the agent starts immediately without requiring a logoff/logon cycle.
...
if ($logonExists -and $updaterExists) {
    ...
    Write-Status "Missing tasks: logon=$logonExists updater=$updaterExists. Recreating."
}
...
# 5. Fire the logon task immediately
# schtasks /Run launches in the logged-on user's session. Returns 1 if no interactive
# session - that is acceptable; the task fires at next logon.
Write-Status "Firing $LogonTaskName ..."
$result = & "$env:SystemRoot\System32\schtasks.exe" /Run /TN $LogonTaskName 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Status "NOTE: schtasks /Run exit $LASTEXITCODE (no interactive session?) - agent starts at next logon. $result"
} else {
    Write-Status "Agent task fired."
}

# 6. Wait briefly then confirm process
...
if ($proc) {
    ...
    Write-Status "NOTE: Agent process not detected after 4s. May start shortly, or no interactive session is active."
    exit 0
}
