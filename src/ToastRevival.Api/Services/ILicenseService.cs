...
public interface ILicenseService
{
    ...
    /// tenants proceed in parallel. Closes the TOCTOU race between
    /// <see cref="CanRegisterDeviceAsync"/> and the device INSERT that
    /// previously let two concurrent trial registrations both pass the
    /// 2-device gate. Returns true when the device was committed; false when
    /// the cap is hit or billing is canceled (no rows written).
    /// </summary>
